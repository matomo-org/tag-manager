    ...
    private function renderManageContainerTemplate($template, $variables = array())
    {
        ...
        foreach ($variables['container']['releases'] as $release) {
            if ($release['environment'] === Environment::ENVIRONMENT_PREVIEW) {
                ...
                $urlInput = '<br><div class="preview-debug-url-div"><input type="url" id="previewDebugUrl" class="browser-default" placeholder="https://yourdebugsite.com" value="'.$debugSiteUrl.'"> <button type="button" class="btn btn-small" onclick=\'tagManagerHelper.changeDebugUrl(' . json_encode($release['idcontainer']) . ',"'.$debugSiteUrl.'")\'>Debug</button></div>';
